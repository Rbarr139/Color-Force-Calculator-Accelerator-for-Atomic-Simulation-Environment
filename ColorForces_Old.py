...
class ColorThermometer(MD):
    ...
    def step(self):
        ...
        if self.counter%50==0:
            print("Current step:", self.counter)
        
    def rescale(self, tol = 1e-5, max_iter = 10):
...
#parent = OCPCalculator
#parent = EMT
#parent = mace_mp(model="small", device='cpu') # Path to downloaded checkpoint

class LayeredColorForceCalculator(Calculator):
    def __init__(self, base_calculator, element, color_force=0.01, direction=[1,0,0], layer_length=2.04, layer_orientation = 'x', **kwargs):
        ...
        self.implemented_properties = set(base_calculator.implemented_properties)
    def calculate(self, atoms=None, properties=['energy'], system_changes=all_changes):
        self.base_calculator.calculate(atoms, properties, system_changes) # Run wrapped calculator first
        self.results = dict(self.base_calculator.results)
